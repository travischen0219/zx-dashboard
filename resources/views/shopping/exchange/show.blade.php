...
<div class="row">
     <form role="form" action="{{ route('s_exchange.search') }}" method="POST">
        {{ csrf_field() }}
        <div class="col-md-12" >
    ...
    <form role="form" action="{{ route('s_exchange.create') }}" method="GET" id="create_from">
        ...
        <div class="portlet light">
            <div class="portlet-title">
                ...
                <div class="caption font-dark">
                    {{-- <a href="{{ route('s_exchange.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> 新增換貨</a> --}}
                </div>
                <div class="tools"> </div>
            </div>
                    ...
                    <tbody>
                         @foreach($exchanges as $exchange)
                            ...
                            <tr>
                                ...
                                <td>S{{$exchange->sale_no}}</td>                            
                                <td>{{$exchange->lot_number}}</td>                            
                                <td>{{$exchange->customer_name->shortName}}</td>
                                <td>{{$exchange->exchangeDate}}</td>
                                <td>{{$exchange->realExchangeDate}}</td>
                                <td>
                                    @if($exchange->sale_name->status == '1') 
                                    <span style="color:red">新訂單</span> 
                                    @elseif($exchange->sale_name->status == '2') 
                                ...
                                <td align="center" id="functions_btn">
                                    @if($exchange->status == 1)
                                        <a href="{{ route('s_exchange.edit', $exchange->id) }}" class="btn blue btn-outline btn-sm">修改</a>
                                        <a href="javascript:;" class="btn red btn-outline btn-sm" onclick="
                                        if(confirm('確定要刪除嗎 ?')){
                                            event.preventDefault();
                                            document.getElementById('delete-form-{{$exchange->id}}').submit();
                                        } else {
                                            event.preventDefault();
                                        }">刪除</a>
                                    <form id="delete-form-{{$exchange->id}}" action="{{ route('s_exchange.destroy', $exchange->id) }}" method="post" style="display:none">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                                    </form>
                                    @elseif($exchange->status == 2)
                                        <a href="{{ route('s_exchange.show', $exchange->id) }}" class="btn purple btn-outline btn-sm">查看</a>
                                    @endif
...
<button id="error_sale_no_prefix" class="btn btn-danger mt-sweetalert" data-title=" 銷貨單號開頭為 P" data-message="" data-allow-outside-click="true" data-confirm-button-class="btn-danger" style="display: none;"></button>
