...
<div class="row">
     <form role="form" action="{{ route('apply_out_stock.search') }}" method="POST">
        {{ csrf_field() }}
        <div class="col-md-12" >
                    ...
                    <div class="col-md-5">
                        ...
                        <div class="col-md-9">
                            <select class="form-control" style="font-size: 14px;" name="search_category">
                                ...
                                <option value="1" {{$search_code == 1 ? 'selected' : ''}}>申請中</option>
                                <option value="2" {{$search_code == 2 ? 'selected' : ''}}>已通過</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-5">
        ...
        <div class="portlet light">
            <div class="portlet-title">
                ...
                <div class="caption font-dark">
                    <a href="{{ route('apply_out_stock.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> 新增申請出庫</a>
                </div>
                <div class="tools"> </div>
            </div>
                         ...
                         @foreach($applies as $apply)
                            ...
                            <tr>
                                ...
                                <td align="center" id="functions_btn">
                                    @if($apply->status == 1)
                                        <a href="{{ route('apply_out_stock.edit', $apply->id) }}" class="btn blue btn-outline btn-sm">修改</a>
                                        <a href="javascript:;" class="btn red btn-outline btn-sm" onclick="
                                            if(confirm('確定要刪除嗎 ?')){
                                                event.preventDefault();
                                                document.getElementById('delete-form-{{$apply->id}}').submit();
                                            } else {
                                                event.preventDefault();
                                            }">刪除</a>
                                        <form id="delete-form-{{$apply->id}}" action="{{ route('apply_out_stock.destroy', $apply->id) }}" method="post" style="display:none">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                        </form>
                                    @elseif($apply->status == 2)
                                        <a href="{{ route('apply_out_stock.show', $apply->id) }}" class="btn purple btn-outline btn-sm">查看</a>
                                    @endif
                                </td>
                            </tr>
