    ...
    <div class="col-md-12 ">
        ...
        <div class="portlet light">
            ...
            <div class="portlet-body form">
                <form role="form" action="{{ route('transfer_inventory.store') }}" method="POST" id="stock_from">
                    {{ csrf_field() }}
                    <div class="form-body">
                        ...
                        <div class="col-md-12" style="height:90px;">
                            ...
                            <div class="col-md-3">
                                <div class="form-group form-md-line-input" >
                                    <input type="text" name="transfer_date" class="form-control" autocomplete="off" id="datepicker01" value="{{ old('transfer_date') }}">
                                    <label for="datepicker01" style="color:#248ff1;">調撥日期 (YYYY-MM-DD)</label>
                                    <span class="help-block"></span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="portlet light bordered">
                                <div class="portlet-title">
                                    <div class="caption font-dark" style="">
                                        <span class="caption-subject"> 調撥單</span>
                                    </div>
                                    <div class="tools"> </div>
                                </div>
                                        ...
                                        <table id="materialTable" class="table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th width="30%"> 物料 </th>
                                                    <th width="10%"> 單位 </th>
                                                    <th width="15%"> 原倉儲位置 </th>
                                                    <th width="15%"> 目前庫存 </th>
                                                    <th width="15%"> 調撥數量 </th>
                                                    <th width="15%"> 新倉儲位置 </th>
                        ...
                        <div class="col-md-12">
                            <div class="form-actions noborder">
                                ...
                                <a href="{{ route('transfer_inventory.index') }}" class="btn red"><i class="fa fa-times"></i> 取 消</a>
                            </div>
                        </div>
                    </div>
...
<button id="error_stock_date" class="btn btn-danger mt-sweetalert" data-title="調撥日期 必填" data-message="" data-allow-outside-click="true" data-confirm-button-class="btn-danger" style="display: none;"></button>
...
<button id="error_same_warehouse" class="btn btn-danger mt-sweetalert" data-title="新倉儲與原倉儲相同" data-message="" data-allow-outside-click="true" data-confirm-button-class="btn-danger" style="display: none;"></button>
<button id="error_quantity" class="btn btn-danger mt-sweetalert" data-title="調撥數量未填寫" data-message="" data-allow-outside-click="true" data-confirm-button-class="btn-danger" style="display: none;"></button>
<button id="error_calculate" class="btn btn-danger mt-sweetalert" data-title="調撥數量大於目前庫存量，請確認數量正確性" data-message="" data-allow-outside-click="true" data-confirm-button-class="btn-danger" style="display: none;"></button>
...
function addMaterial() {
    $.post(
        "{{ route('select_material_transfer_inventory.addRow') }}",
...
function openSelectNewWarehouse(id) {
    currentMaterial = id;
    if($('#material' + currentMaterial).val()){
        var material_id = $('#material' + currentMaterial).val();
        var url_str = 'settings/selectNewWarehouse_stock/'+material_id;
        $.magnificPopup.open({
            showCloseBtn : false,
            enableEscapeKey : false,
            closeOnBgClick: true,
...
function setNewWarehouse(id,fullName,code,category){
    $.magnificPopup.close();
    $('#materialNewWarehouseName' + currentMaterial).html(code);
    $('#materialNewWarehouse' + currentMaterial).val(id);
}
...
function submit_btn(){
    ...
    var check_new_warehouse = 0;
    var check_same_warehouse = 0;
    ...
    $(".materialRow").each(function(index, el) {
        ...
        if($(this).find(".select_materialNewWarehouse").val() == ''){
            check_new_warehouse++;
        }
        ...
        if($(this).find(".select_materialWarehouse").val() == $(this).find(".select_materialNewWarehouse").val()){
            check_same_warehouse++;
        }
    });
    if(check_material == 0){
    ...
    if(check_new_warehouse > 0){
        $('#error_warehouse').click();
        return;
    }
    if(check_same_warehouse > 0){
        $('#error_same_warehouse').click();
        return;
    }
    if(check_quantity == 0){
