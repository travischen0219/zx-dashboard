...
<div class="row">
    <div class="col-md-12" style="margin-top:25px;">
        {{-- <div class="col-md-5">
            <label class="col-md-3 control-label" style="color:#248ff1;font-size: 16px;line-height: 32px;text-align: center">條碼輸入 :</label>
            <div class="col-md-9">
                <input type="text" class="form-control" name="search_barcode" id="search_barcode" onchange="search_input();">
            </div>
        </div>  --}}
    </div>
                    ...
                    <tbody>
                        @foreach($materials as $material)
                            ...
                            <tr>
                                ...
                                {{-- @if($material->material_name->safe >0)
                                    <td>{{$material->material_name->safe}}</td>
                                @else
                                    <td><span style="color:red;">未設定</span></td> 
                                @endif --}}

                                {{-- @if($material->material_name->safe >= $material->material_name->stock )
                                    <td style="border-left-color:red;border-width:1px;">
                                    <font color=red>{{$material->material_name->stock}}</font>
                                @else --}}
                                    <td align="right">
                                ...
                                <td align="center">
                                    <span style="display:none">{{ $material->physical_inventory > 0 ? $material->physical_inventory : ''}}</span>
                                    <form action="{{ route('inventory_list.update',$material->id) }}" method="POST">
                                        {{ csrf_field() }}
                                        {{ method_field('PUT') }}                                                
                                        <input type="text" name="p_inventory" style="background-color:lightblue;border-color:blue;border-width:1px;line-height: 30px;text-align: center;" value="{{ $material->physical_inventory > 0 ? $material->physical_inventory : ''}}">
                                        <button class="btn" style="color:#fff;background-color: #248ff1;margin-top:-3px;">存檔</button>
                                    </form>
                                </td>
                                <td align="center">
